function Show-MenuFunctions 
{
  Begin{
    #####   MENU TESTING #####
    Clear-Host
    $choice = ''
    $MenuTitle = 'Menu Function Examples'
    $MenuList = (@'
{1}
...
'@  -f $MenuTitle, $('-'*50) )
    ...
    function Script:Write_Out
    {
      param([Parameter(Mandatory = $true)][String]$InputObject)
      Write-Host -Object $InputObject -ForegroundColor Cyan
      $InputObject | & "$env:windir\system32\clip.exe"
      Write-Host -Object 'The above has been copied to the clipboard' -ForegroundColor Yellow
  ...
  Process{
    while ($choice -ne 0)
    {
      ...
      switch ($choice)
      {
        ...
        1 
        {
          Write_Out -InputObject $quickMenu
        }
        2
        {
          Write_Out -InputObject $MainMenu
        }
        3
        {
          Write_Out -InputObject $InputBoxButtonClick
        }
        4
        {
          Write_Out -InputObject $GuiMenu
        }
        5
        {
          Write_Out -InputObject $DynamicMenu
        }
        6
        {
          Write_Out -InputObject $AsciiMenu
        }
        7
        {
          Write_Out -InputObject $OneClickGuiMenu
        }
        8
        {
          Write_Out -InputObject $ChoiceGuiMenu
        }
        default 
        {
